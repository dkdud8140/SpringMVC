...
public class MyController {
	 ...
	 * 
	 *  BESTfull HTTP SErver에서 추가하여 사용
	 *  	PUT : 업데이트(UPDATE)
	 *  	DELETE : 석제(DELETE)
	 *  
	 *  RequsetMapping이 같고, RequestMethod가 같은 경우는
	 *  절대 허용하지 않는다
	 *  
	 *  RequestMapping이 같더라도 RequestMethod가 다르면
	 *  같이 사용할 수 있다.
	 *  
	 *  GET 메소드 방식으로 form에 담긴 데이터를 전송할 
	 *  http://localhost:8080/hello/insert?name=홍길동&tel=전화번호
	 * 	-전송하는 데이터가 주소에 포함되엇 전송되기 때문에
	 *	데이터가 쉽게 노춛될 수 있다
	 *- 전체 URI(주소, 데이터 등등)의 길이가 256BYTE를 넘어가면 데이터가 잘리는 현상이 발생한다.
	 *
	 * 
	 * 
	 * 	POST 메소드 방식으로 form에 담긴 데이터를 전송할 때
	 * http://localhost:8080/hello/insert	
	 * -전송하는 데이터가 HTTP 프로토콜의 BODY에 담겨서
	 * 		전송되기 때문에 일단 주소차 등에 노출이 되지 않는다
	 * - 이론상으로는 전송하는 데이터의 ㄱ크기에 제한이 없다
	 ...
	 *  요청을 받았으면
	 *  @RequestMapping(value="/insert",method=GET)를 처리하는 메소드가 있어야 한다
	 *  
	 *  Mapping(value="/insert")가 있는데
	 *  메소드가 POST만 있을 경우 서버는 405 Http 상태코드를 응답한다.
	 *  
	 *  
	 *  
	 *  
	 ...
	 * getParemeter()등의 코드를 이젠은 잊어도 된다
	 * 
	 * 그런데
	 * Java Method의 변수를 String형이 아닌 것으로(숫자형, boolean형 등)
	 * 설정을 하면 Reflection이 실행되는 동안에 String형 데이터를
	 * 설정된 데이터 type으로 형 변환 과정이 실행된다
	 * 이 과정에서 형변환의 exception이 발생하면 Reflection 코드가 중단되고
	 * Spring의 Dispatcher는 Web 400 status code를 응답해 ㅓㅂ린다
	 * 
	 * 또한 구체적인 어떤 문제가 발생하였는지 알려주지 않는다
	 * 
	 *
	 */
