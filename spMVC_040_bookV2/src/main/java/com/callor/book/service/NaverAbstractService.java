 ...
 * 1. 상속되는 부모 클래스의 역할
 * 		스스로는 객체로 생성될 수 없다.
 * 2. 추상메소드를 포함하는데 상속받은 곳에서는 반드시 추상메소드를 구현해야한다.
 * 3. 상속받은 클래스들에 필요한 공통 처리 method는 일반 클래스처럼 자체적으로 구현 코드를 가지고 있다.
 * 4. 공통 처리 method도 필요에 따라 상속받는 곳에서 재 정의 할 수 있다.
 * 
 * 5. 인터페이스 역할을 수행
 * 		여러 클래스의 원형(prototype) 역할을 수행하면서 
 * 		상속받은 클래슫ㄹ에 공통으로 필요한 코드를 미리 제공한다.
 * 
 * 객체 생성하는 방법
