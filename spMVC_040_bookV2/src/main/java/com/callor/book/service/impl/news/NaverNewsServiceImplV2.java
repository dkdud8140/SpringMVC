...
/*
 * spring framework의 RsonTemplate 클래스를 사용한 JSON parsing
 * 
 ...
 * HttpEntity로 변환허여 RestTemplate에 주입한다
 * 
 ...
 * 내부에서
 * 요청정보를 Open API로 보내고
 * 수신된 데이터를 DTO(VO)구조에 따라 자세 parsing을 하고
 * 결과를 돌려준다
 */
...
public class NaverNewsServiceImplV2 extends NaverNewsServiceImplV1{
	...
	public List<NewsDTO> getNaverList(String queryURL) throws Exception {
		...
		/*
		 * naver에 요청할 때 사용할 인증 정보를 http protocol의 header에 부착하기
		 */
		HttpHeaders header = new HttpHeaders();
		header.set("X-Naver-Client_Id",NaverSecret.NAVER_CLIENT_ID);
		header.set("X-Naver-Client_Secret",NaverSecret.NAVER_CLIENT_SECRET);
		...
		List<NewsDTO> news_list = resList.getBody().items;
