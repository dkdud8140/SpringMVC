...
<style>
	...
	/*
		박스내의 텍스트의 그려지는 높이를 박스의 높이와 같게 만들면 테스트가 박스의 세로방향 가운데 정렬이 된다
	*/
...
/*
const : 한 번 설정된 변수는 다시 설정되지 않는다
		JS에서 상수를 선언하기

다른 언어에서는 상수 선언이 메모ㅈ리적 문제를 해결하고
동시성처리(멀티 환경에서 서로 변수가 간섭하는 현상을 핸들링)을 쉽게 하기 위한 방안으로 사용

상수를 선언하는 이유
여기에 설정된 값이 코드 중간에 어떤 이유로 변경되는 것을 방지하는 역할

한 개의 선언되 변수에 코드 중간에 다른 값이 저장되어(의도하든 그렇지 않든)
논리적인 오류를 일으킬 수 있다.
