...
public class MemberServiceImplV1 implements MemberService{
	 ...
	 * 		회원테이블에 데이터가 있냐 없냐?
	 * 		selectAll() method를 사용하여 최초 가입된 
	 * 2. ADMIN 권한을 갖는 최초의 가입자는 level 0이다
	 * 3. ADMIN이 아닌 일반 가입자는 기본 level이 9이다
	 * 4. level 6보다 큰 member는 이미지 보기만 가능하다
	...
	public MemberVO findById(String m_userid) {
		MemberVO member = mDao.findById(m_userid.trim());
		
		if(member == null) {
			// 가입되지 않은 사용자 ID
			log.debug(" 가입되지 않은 사용자");
		} else {
			
			log.debug("조회된 사용자 id : {}" , m_userid);
		}
