...
public class FileServiceImplV2 extends FileServiceImplV1 {
	...
	// 전달받은 파일이름으로 된 파일을 fileUpaPath에 저장된 파일 삭제
	@Override
	public int delete(String imgFileName) {

		/*
		 * fileService.delete(파일명)을 호출할 때
		 * 파일명이 null이거나 없으면 진행을 멈추는 코드
		 * 
		 * 이러한 코드는 호출하는 곳에서
		 * if(파일명 == null || 파일면.isEmpty()) {
		 * 	fileService.delete(파일명)
		 * }
		 * 
		 * 처럼 호출할 수 있지만
		 * delete() method 사용하는 곳이 여러봇이면
		...
		if(delFile.exists()) {
			...
			if(ok) {
				 ...
				 * 실패하는 메시지를 return할 때 null을 return하면 된다
				 * 
				 * primitive 숫자형으로 return type을 설정한 경우
				 * null값을 return할 수 없다
				 * 
				 * 이럴 때 성공한 메시지는 양의 정수값을 return하고
				 * 실패한 메시지는 음의 정수값을 return한다
				 * 
				 * 호출한 곳에서 성공, 실패를 검서헐 땨
				 * if(return > 0) 성공
