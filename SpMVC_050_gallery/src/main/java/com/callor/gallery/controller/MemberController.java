...
public class MemberController {
	...
	public String join(Model model) {
		...
		model.addAttribute("BODY", "GA-JOIN");
		return "home";
	}
	...
	public String id_check(String m_userid) {
		...
		MemberVO memberVO = mSer.findById(m_userid);
	...
	public String login( MemberVO memberVO, Model model, HttpSession session) {
		...
		memberVO = mSer.login(memberVO,model);
		if(memberVO == null) {
			model.addAttribute("BODY","LOGIN");
			return "home";
		} else {
			 ...
			 * 임의로 삭제하거나, 초기화하거나, 서버가 멈추거나, 일정조건이 성립되지 않으면
			 * 서버 메모리에 영구히 남아있다
			 * 
