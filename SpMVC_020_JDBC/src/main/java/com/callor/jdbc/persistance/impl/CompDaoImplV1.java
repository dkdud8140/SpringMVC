...
public class CompDaoImplV1 implements CompDao {
	 ...
	 * 각 메소드에 매개변수를 받아 이 쿼리에 전달할 텐데
	 * 이 때 매겨변수를 primitive르 받으면 값을 변환시키는 어려움이 있다
	 * 권장사항으로 매개견수는 wrapper class type으로 설정
	 * 즉 숫자형일 경우 Integer, Long형으로 선언
	 * 
	 * vo에 담겨서 전달된 값은 Object[] 배열을 jdbcTemplate에 전달해주어야 한다
	 * 
	 * 하지만, 1~2개의 값을 전달할 때 Object[] 배열로 변환 후
	 * 전달을 하면 오블젝트 객체 저장소가 생성되고 메모리를 사용한단
	 * 이 떄 전달되는 변수가 wrapper class typedlaus 
